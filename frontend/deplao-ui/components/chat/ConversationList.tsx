...
import { useChatStore } from '@deplao/store/chatStore';
import { useAccountStore } from '@deplao/store/accountStore';
import { useAppStore } from '@deplao/store/appStore';
import ipc from '@deplao/lib/ipc';
import { sendSeenForThread } from '@deplao/lib/sendSeenHelper';
import { getFilteredUnreadCount } from '@deplao/lib/badgeUtils';
...
import { toLocalMediaUrl } from '@deplao/lib/localMedia';
import GlobalSearchPanel from './GlobalSearchPanel';
import useIsMobile from '@deplao/hooks/useIsMobile';
import { syncZaloGroups, MemberPlaceholder } from '@deplao/lib/zaloGroupUtils';
import { ChannelBadgeOverlay } from '../common/ChannelBadge';
import { getCapability, channelSupports, type Channel } from '@deplao/../configs/channelConfig';
...
export default function ConversationList() {
  ...
  const mergedContacts = useMemo(() => {
    ...
    const result: import('@deplao/store/chatStore').ContactItem[] = [];
    for (const zaloId of mergedInboxAccounts) {
  ...
  const doPhoneSearch = async (acc: import('@deplao/store/accountStore').AccountInfo, phone: string) => {
  ...
  const handleSearchChange = async (value: string) => {
    ...
    let acc: import('@deplao/store/accountStore').AccountInfo | undefined;
    if (mergedInboxMode && mergedInboxFilterAccount) {
  ...
  const handleMergedClick = async (contact: import('@deplao/store/chatStore').ContactItem) => {
