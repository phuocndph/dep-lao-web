...
import { useVisibleAccounts } from '@deplao/hooks/useVisibleAccounts';
import { hasUnseenSettingsTabs } from '@deplao/utils/settingsSeenTabs';
import { useErpPermissions } from '@deplao/hooks/erp/useErpContext';
...
export default function Sidebar({ onAddAccount }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col w-16 bg-gray-900 border-r border-gray-700 h-full">
      ...
      <div className="flex-1 overflow-y-auto py-2 flex flex-col items-center gap-2">
        {(accounts ?? []).filter(Boolean).map((account, index) => {
