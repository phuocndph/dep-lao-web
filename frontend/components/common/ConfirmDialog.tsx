...
export function ConfirmDialog({
  ...
  return (
    ...
    >
      <div className="bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-gray-700/60">
          <button onClick={onCancel} className="px-4 py-2 rounded-xl text-sm font-medium text-gray-300 hover:text-white hover:bg-gray-700 transition-colors">
            {cancelText}
          </button>
          <button onClick={onConfirm} autoFocus className={`px-5 py-2 rounded-xl text-sm font-semibold transition-colors ${confirmBtnClass[variant]}`}>
            {confirmText}
          </button>
        </div>
      </div>
...
export function showConfirm(options: ConfirmOptions): Promise<boolean> {
  return new Promise(resolve => {
    ...
    root.render(
      <ConfirmDialog {...options} open onConfirm={() => cleanup(true)} onCancel={() => cleanup(false)} />
    )
  })
}
...
export function useConfirm() {
  ...
  const handleConfirm = useCallback(() => { ref.current?.resolve(true); setState(null) }, [])
  const handleCancel  = useCallback(() => { ref.current?.resolve(false); setState(null) }, [])

  const ConfirmModal = state ? (
    <ConfirmDialog {...state} open onConfirm={handleConfirm} onCancel={handleCancel} />
  ) : null
