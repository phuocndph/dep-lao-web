...
import type { CRMNote } from '@/stores/crmStore'
...
export default function NoteList({ notes, onSave, onDelete }: NoteListProps) {
  ...
  const fmt = (ts: number) =>
    ts ? new Date(ts).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''
          ...
          {editId === note.id ? (
            <div className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                <button onClick={() => setEditId(null)} className="flex-1 py-1 rounded bg-gray-600 text-xs text-gray-300 hover:bg-gray-500">
                  Hủy
                </button>
                {/* TODO: note update endpoint not yet implemented — creates new note */}
                <button onClick={() => handleEdit(note)} disabled={saving} className="flex-1 py-1 rounded bg-blue-600 text-xs text-white hover:bg-blue-700 disabled:opacity-50">
                  Lưu
                </button>
              </div>
            </div>
            ...
            <>
              ...
              <div className="flex items-center justify-between mt-1.5">
                ...
                <div className="opacity-0 group-hover:opacity-100 flex gap-2 transition-opacity">
                  <button onClick={() => { setEditId(note.id); setEditText(note.content) }} className="text-[11px] text-blue-400 hover:text-blue-300">
                    Sửa
                  </button>
                  <button onClick={() => onDelete(note.id)} className="text-[11px] text-red-400 hover:text-red-300">
                    Xóa
                  </button>
                </div>
              </div>
