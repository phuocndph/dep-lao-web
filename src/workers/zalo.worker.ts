...
async function loginQR(): Promise<void> {
  ...
  try {
    // loginQR on the Zalo class resolves to API directly once the user scans
  ...
  if (!resolvedApi) {
    emit({ type: 'LOGIN_FAILED', error: 'loginQR returned null' })
    return
  }
...
async function handleCommand(cmd: WorkerCommand): Promise<void> {
  switch (cmd.type) {
    ...
    case 'SEND_MESSAGE': {
      if (!api) {
        emit({ type: 'MESSAGE_ERROR', error: 'API not ready', context: { cmd } })
        return
      }
      if (!checkRateLimit()) {
        emit({
          type: 'MESSAGE_ERROR',
          error: `Rate limit exceeded: max ${MAX_MSG_PER_MINUTE} messages/minute`,
          context: { threadId: cmd.payload.threadId },
        })
        return
      }
      const { threadId, threadType, content, quoteMessageId: _q } = cmd.payload
...
// Signal to pool manager that worker is alive and waiting for commands
emit({ type: 'HEARTBEAT_ACK', timestamp: Date.now() })
