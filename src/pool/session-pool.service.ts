...
const WORKER_SCRIPT = resolve(__dirname, '../workers/zalo.worker.js')
...
export class SessionPoolService {
  ...
  async addAccount(params: {
    ...
    if (this.processes.size >= this.config.maxWorkers) {
    ...
    // tenantId stored separately — SessionRecord has no tenantId field
    ...
    if (vaultTokenId) {
      await this.sendCommand(accountId, {
        type: 'LOGIN_COOKIE',
        // Worker reads real creds from vault via VAULT_TOKEN_ID env
        payload: { cookie: {}, imei: '', userAgent: '' },
      })
    } else {
  ...
  async sendCommand(accountId: string, command: WorkerCommand): Promise<void> {
    const proc = this.processes.get(accountId)
    if (!proc || proc.killed) {
      throw new Error(`No active worker for account ${accountId}`)
    }
    proc.send(command)
  }
  ...
  private spawnWorker(
    ...
    proc.on('exit', (code, signal) => {
      this.handleWorkerExit(accountId, code, signal)
    })

    proc.stdout?.on('data', (d: Buffer) => {
      process.stdout.write(`[W:${accountId.slice(0, 8)}] ${d}`)
    })

    proc.stderr?.on('data', (d: Buffer) => {
      process.stderr.write(`[W:${accountId.slice(0, 8)}] ${d}`)
    })
    ...
    return new Promise<void>((res, rej) => {
      ...
      const poll = setInterval(() => {
        ...
        if (Date.now() > deadline) {
          clearInterval(poll)
          rej(new Error(`Worker ${accountId} did not become ready within ${ms}ms`))
        }
      }, 200)
    })
  }
  ...
  private async handleWorkerEvent(accountId: string, event: WorkerEvent): Promise<void> {
    ...
    switch (event.type) {
      case 'HEARTBEAT_ACK': {
        ...
        // First HEARTBEAT_ACK signals the process is alive and accepting commands
      ...
      case 'QR_CODE': {
        ...
        await this.redisPub.publish(
          'zalo:status',
          JSON.stringify({ event: 'QR_CODE', accountId, qrDataUrl: event.payload.qrDataUrl }),
        )
        break
      }
      ...
      case 'LOGIN_SUCCESS': {
        ...
        await this.redisPub.publish(
          'zalo:status',
          JSON.stringify({ event: 'LOGIN_SUCCESS', accountId, ...event.payload }),
        )
        break
      }
      ...
      case 'LOGIN_FAILED': {
        ...
        await this.redisPub.publish(
          'zalo:status',
          JSON.stringify({ event: 'LOGIN_FAILED', accountId, error: event.error }),
        )
        break
      }
      ...
      case 'MESSAGE_INCOMING': {
        await this.redisPub.publish(
          'zalo:messages',
          JSON.stringify({ accountId, workerId: session.workerId, message: event.payload }),
        )
        break
      }
  ...
  private handleWorkerExit(
    ...
    console.warn(
      `[pool] Worker ${accountId.slice(0, 8)} exited (code=${code}, signal=${signal}) — scheduling restart`,
    )
  ...
  private async reconnect(accountId: string): Promise<void> {
    ...
    if (storedCreds) {
      const creds = JSON.parse(storedCreds) as {
      ...
      await this.sendCommand(accountId, {
        type: 'LOGIN_COOKIE',
        payload: { cookie: creds.cookie, imei: creds.imei, userAgent: creds.userAgent },
      })
    } else {
      await this.sendCommand(accountId, { type: 'LOGIN_QR' })
    }
  ...
  private async runHealthCheck(): Promise<void> {
    for (const [accountId, session] of this.sessions) {
      ...
      if (staleness > this.config.heartbeatIntervalMs * 3) {
        ...
        try {
          await this.sendCommand(accountId, { type: 'HEARTBEAT' })
